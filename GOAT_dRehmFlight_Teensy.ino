//Project GOAT - CREATE Lab
//Author: Guillaume Hueber
//Project Start: 10/09/2024
...
float AccErrorZ = 0.53;
float GyroErrorX = -9.20;
float GyroErrorY = 0.39;
...
float maxYaw = 15.0;     //Max yaw rate in deg/sec, Default = 160

float Kp_roll_angle = 0.25;    //Roll P-gain - angle mode 
//float Ki_roll_angle = 0.00;    //Roll I-gain - angle mode //Was 0.3
float Kd_roll_angle = 0.01;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
...
//Flight status
bool armedFly = false;
bool flight_mode = 0; // 0 means drone, 1 means plane
...
// control gains
// hover//////////////////////////////////////:
float kp_pitch_angle_hover = 0.3;
float kd_pitch_angle_hover = 0.05;
float kp_roll_angle_hover =  0.5;
float kd_roll_angle_hover = 0.05;
float kp_yaw_hover = 0.2; 
...
float kp_roll_angle_ff = 0.02;
float Ki_roll_angle = 0.005;    //Roll I-gain - angle mode //Was 0.3
float kd_roll_angle_ff = 0.001;

float kp_yaw_ff = 0.7; //Was 0.02
float Ki_yaw = 0.005;          //Yaw I-gain
float kd_yaw_ff = 0.00017;
/////////////////////////////////////////
// servo trims
float servo_trim_1 = -0.01;
float servo_trim_2 = 0.04;
...
void loop() {
  ...
  print_pitch_roll_stuff();

  // Get arming status
  armedStatus(); //Check if the throttle cut is off and throttle is low.

  if(armedFly){ digitalWrite(13, HIGH); //Pin 13 is built in LED
  } else{
  ...
  // fade the gains
  Kp_roll_angle = (transition_fader)*(kp_roll_angle_ff) + (1.0 - transition_fader)*(kp_roll_angle_hover); // (1.0 - transition_fader)*(forward flight) + (transition_fader)*(hover)
  Kd_roll_angle = (transition_fader)*(kd_roll_angle_ff) + (1.0 - transition_fader)*(kd_roll_angle_hover);
  Kp_pitch_angle = (transition_fader)*(kp_pitch_angle_ff) + (1.0 - transition_fader)*(kp_pitch_angle_hover);
  Kd_pitch_angle = (transition_fader)*(kd_pitch_angle_ff) + (1.0 - transition_fader)*(kd_pitch_angle_hover);
  Kp_yaw = (transition_fader)*(kp_yaw_ff) + (1.0 - transition_fader)*(kp_yaw_hover);
  Kd_yaw = (transition_fader)*(kd_yaw_ff) + (1.0 - transition_fader)*(kd_yaw_hover);
...
void controlMixer() {
  ...
  //GOAT mixing
  //m1_command_scaled = thro_des + abs(pitch_PID) + roll_PID; //Front Left
  m1_command_scaled = thro_des + roll_PID; //Front Left
  m2_command_scaled = thro_des - roll_PID; //Front Right
  s1_command_scaled = 0.5 + servo_trim_1 + pitch_PID + yaw_PID;
  s2_command_scaled = 0.5 + servo_trim_2 - pitch_PID + yaw_PID;


/*
  // bicopter tailsitter
  //(1.0 - transition_fader)*(forward flight) + (transition_fader)*(hover)
  m1_command_scaled = (1.0 - transition_fader)*(thro_des + yaw_PID) + (transition_fader)*(thro_des + abs(pitch_PID) + roll_PID - yaw_PID); // right
  m2_command_scaled = (1.0 - transition_fader)*(thro_des - yaw_PID) + (transition_fader)*(thro_des + abs(pitch_PID) - roll_PID + yaw_PID); // left
...
*/
  //s1_command_scaled = 0.5 - pitch_PID;
  /*s2_command_scaled = 0;
  s3_command_scaled = 0;
  s4_command_scaled = 0;
  s5_command_scaled = 0;
...
void Madgwick6DOF(float gx, float gy, float gz, float ax, float ay, float az, float invSampleFreq) {
  ...
  yaw_IMU = -atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3)*57.29577951; //degrees
  */
  //if(flight_mode == 1) pitch_IMU = pitch_IMU - 90;

  /*
  //Change Euler angle ordering based on flight mode for tailsitters
  //SJM 5-3-22
  if (channel_6_pwm > 1500) { //cruise mode
  ...
  yaw_IMU = (1.0 - transition_fader)*(atan2(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2)*57.29577951) + (transition_fader)*(asin(constrain(2.0f * (q0*q1 + q2*q3),-0.99999,0.99999))*57.29577951); // (1.0 - transition_fader)*(ff) + (transition_fader)*(hover)
  ...
  roll_IMU = (1.0 - transition_fader)*(-atan2(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3)*57.29577951) + (transition_fader)*(-atan2(-q1*q2 + q0*q3, 0.5f - (q1*q1 + q3*q3))*57.29577951);
  float temp = GyroZ;
...
void controlANGLE() {
  ...
  //Pitch
  /*if(flight_mode = 1) error_pitch = pitch_des - (pitch_IMU - 90);
  else error_pitch = pitch_des - pitch_IMU;
  */
...
void getCommands() {
   ...
   * The raw radio commands are filtered with a first order low-pass filter to eliminate any really high frequency noise. 
   ...
   unsigned long channel_3_fs = 1500; //elev
   unsigned long channel_4_fs = 1500; //rudd
   unsigned long channel_5_fs = 1000; //gear, lower than 1500 = throttle cut 
   */

  #if defined USE_PPM_RX || defined USE_PWM_RX
    channel_1_pwm = getRadioPWM(3);
    channel_2_pwm = getRadioPWM(1);
    channel_3_pwm = getRadioPWM(2);
